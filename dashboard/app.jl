...
app.layout = html_div(style=Dict("fontFamily" => "Inter, sans-serif", "backgroundColor" => "#F7FAFA", "minHeight" => "100vh", "padding" => "2rem")) do
    html_div(style=Dict("maxWidth" => "1200px", "margin" => "0 auto")) do
        html_div(style=Dict("textAlign" => "center", "marginBottom" => "3rem")) do
            ...
            html_p("Deterministic PBPK Engine", style=Dict("color" => "#0FA3B1", "fontSize" => "1.2rem", "fontWeight" => "300"))
        end,
        
        html_div(style=Dict("display" => "grid", "gridTemplateColumns" => "1fr 2fr", "gap" => "2rem")) do
...
callback!(
    ...
    layout = Layout(
        title=attr(text="Pharmacokinetic Profile", font=attr(family="Inter", color="#0E2A3A", size=24)),
